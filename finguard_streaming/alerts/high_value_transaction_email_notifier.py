...
# Email notifier script for transaction beyond a customer limit, using sink and append_flow
# data source is : finguard.gold.transaction
...
def send_high_value_alert_emails(df, batch_id):
    ...
    if APP_PASSWORD is None:
        print(
            f"❌ Batch {batch_id}: Gmail API key not available, skipping email notifications"
        )
        return
    ...
    for row in rows:
        try:
            ...
            success_count += 1
            print(
                f"  ✅ Email sent to {row.customer_email} for transaction {row.transaction_id}"
            )
    ...
    print(
        f"📊 Batch {batch_id} complete: {success_count} succeeded, {failure_count} failed"
    )
...
def high_value_alert_stream():
    ...
    return spark.readStream.table("finguard.gold.transaction")
